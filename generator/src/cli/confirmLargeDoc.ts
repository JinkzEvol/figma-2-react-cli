...
export function confirmLargeDocument(context: { nodeCount: number; depth: number; thresholds: { nodeCount: number; depth: number } }): LargeDocDecision {
  if (process.env.LARGE_DOC_AUTO_ACCEPT === '1') return { accepted: true };
  if (process.env.LARGE_DOC_AUTO_DECLINE === '1') return { accepted: false, reason: 'Env decline' };
