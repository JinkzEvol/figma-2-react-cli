...
async function runRetention(options) {
    ...
    if (!dryRun) {
        ...
        for (const candidate of candidates) {
            await node_fs_1.promises.rm(candidate.path, { recursive: true, force: true });
...
async function loadVersionEntries(rootDir, now) {
    const dirents = await node_fs_1.promises.readdir(rootDir, { withFileTypes: true });
    const entries = [];
    for (const dirent of dirents) {
        ...
        entries.push({
            name: dirent.name,
            path: node_path_1.default.join(rootDir, dirent.name),
            timestamp,
            ageDays
        });
    }
    ...
    (async () => {
        try {
            ...
            const result = await runRetention({
                rootDir: node_path_1.default.resolve(parsed.rootDir),
