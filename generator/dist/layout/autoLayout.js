...
exports.mapAutoLayout = mapAutoLayout;
...
function alignToClass(axis, value) {
    if (!value)
        return undefined;
    switch (value) {
        case 'MIN':
...
function paddingToClasses(p) {
    ...
    const out = [];
    if (top)
        out.push(`pt-[${Math.round(top)}px]`);
    if (right)
        out.push(`pr-[${Math.round(right)}px]`);
    if (bottom)
        out.push(`pb-[${Math.round(bottom)}px]`);
    if (left)
        out.push(`pl-[${Math.round(left)}px]`);
    return out;
}
function mapAutoLayout(props) {
    const classes = [];
